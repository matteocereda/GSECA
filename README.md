...
## 7. How to run GSECA from command line
...
An example dataset is provided, which contains 50 prostate adenocarcinoma RNA-seq samples  of the TCGA-PRAD dataset, obtained from the GDC data portal (https://gdc-portal.nci.nih.gov). The samples are stratified for the somatic loss of PTEN.
...
2. Read input files 
> M  <- read.delim("Examples/PRAD.ptenloss.M.tsv", stringsAsFactors = F) # gene expression matrix
> L  <- read.delim("Examples/PRAD.ptenloss.L.tsv", stringsAsFactors = F)[,1] # sample type labels
> pl <- read.gmt.file("gene:sets/ncomm.cereda.186.KEGG.gmt")

3. Prepare dataset
> expr <- get_expression_dataset(M, L, 'symbol')

4. FMM and data discretization
...
> ecmap = GSECA.ECmap( gseca
                     , filename=paste0("Figures/gseca_PTEN.sumlog.psumlog_",
                                                            PSUMLOG,"_padj_",
                                                            PADJ,"_pw_",
                                                            PW,"_pemp_",
                                                            PEMP,"_srate_",
...
8. Save Results
> save(expr, gcr, gseca, file="Results/gseca.PRAD.ptenloss.Rdata")
> write.csv(gseca, file="Results/gseca.PRAD.ptenloss.csv")
